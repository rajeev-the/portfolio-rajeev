import React, { useState } from 'react'

const Projects = () => {

  const [first, setFirst] = useState(false);
  const [second, setSecond] = useState(false);
  const [third, setThird] = useState(false);

  const handleChange = () => {
    setFirst(!first);

  }

  const handleChange2 = () => {
    setSecond(!second)

  }
  const handleChange3 = () => {
    setThird(!third)

  }

  

console.log(first)
  return (
   
    <div className='  h-auto w-full   bg-[linear-gradient(-90deg,rgb(26,106,255),rgb(34,58,102))]'>
    
    <div className=' flex flex-col'>

      <div className='    flex  flex-col items-center justify-center gap-4'>

        <div className='h-[100vh] w-full flex  flex-col items-center justify-center  m-2  gap-1'> 

        <h1 className='  text-2xl font-bold   mt-4  text-white '>Mern Stack Developer</h1>

        <div className='h-full w-full flex  items-center justify-evenly  '>   
        <button className='' onClick={handleChange}  ><img src="data:image/png;base64,"/></button>
             { first ? <div  className='  h-[85%]  bg-black   w-[70%]  rounded-[50px]  flex items-center   justify-center'>
  
              <iframe class="rounded-[50px]" width="100%" height="100%" src="https://www.youtube.com/embed/iaclKgCKPOY?si=RCRFi3TcP2lyJYxv&autoplay=1&mute=1&loop=1&playlist=iaclKgCKPOY&rel=0" title="YouTube video player" ></iframe>


     </div>  : <div className=' h-[85%] bg-black  w-[70%]   rounded-[50px]'> 
      
     <iframe  class="rounded-[50px]" width="100%" height="100%" src="https://www.youtube.com/embed/ofSyI6V2Cwo?si=H5Nqv1koM6AZnqFX&loop=1&autoplay=1&mute=1&playlist=ofSyI6V2Cwo&modestbranding=1&showinfo=0&rel=0" title="YouTube video player" ></iframe>

       </div> } 
         <button className='' onClick={handleChange} ><img src="data:image/png;base64,"/></button>
         </div>
           </div>

           <div className='h-[100vh] w-full flex  flex-col items-center justify-center  m-2  gap-1'> 

<h1 className='  text-2xl font-bold  mt-4   text-white '>APP Developer</h1>

<div className='h-full w-full flex  items-center justify-evenly '>   
<button className='' onClick={handleChange2}  ><img src="data:image/png;base64,"/></button>
   { second ? <div className=' h-[85%] bg-white  w-[70%]   rounded-[50px]  flex items-center justify-center text-black text-xl '>  Available Soon </div>   : <div className=' h-[85%] bg-white  w-[70%]  flex items-center justify-center text-black text-xl  rounded-[50px]'> Available Soon  </div>  }  
 <button className='' onClick={handleChange2} ><img src="data:image/png;base64,"/></button>
 </div>
           </div>

           <div className='h-[100vh] w-full flex  flex-col items-center justify-center  m-2  gap-1'> 

<h1 className='  text-2xl font-bold  mt-4   text-white '>UI/UX Developer</h1>

<div className='h-full w-full flex  items-center justify-evenly '>   
<button className='' onClick={handleChange3}  ><img src="data:image/png;base64,"/></button>
   { third ? <div  className='  h-[85%]  bg-black   w-[70%]  rounded-[50px]  flex items-center   justify-center'>
  
    <iframe class="rounded-[50px]" width="100%" height="100%" src="https://www.youtube.com/embed/FNL4j3ETqxo?autoplay=1&mute=1&loop=1&playlist=FNL4j3ETqxo&rel=0"></iframe>

       </div>   : <div className=' h-[85%] bg-black  w-[70%]   flex items-center   justify-center  rounded-[50px]'>
        
        
       <iframe class="rounded-[50px]" width="100%" height="100%" src="https://www.youtube.com/embed/AsJ-ZULdVrA?autoplay=1&mute=1&playlist=AsJ-ZULdVrA&loop=1&rel=0" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>






      

        
           </div>  }  
 <button className='' onClick={handleChange3} ><img src="data:image/png;base64,"/></button>
 </div>
           </div>
        

      </div>

    

   



    </div>
    
    </div>
  )
}

export default Projects